...
const StepsComponent = ({ steps, activeStep, onStepPress }: Props) => {
    ...
    return (
        <div className='flex justify-between items-center space-x-1 w-full p-1 mb-2'>
                ...
                return (
                    <div key={`step_${index}_${step.label}`} className='flex justify-center items-center space-x-2 w-full'>
                        <div className='flex flex-col items-center w-full'>
                            ...
                            >
                                <p className='font-medium truncate'><span className='text-semibold'>{index + 1}. </span>{step.label}</p>
                            </button>
                        </div>
                    </div>
                )
