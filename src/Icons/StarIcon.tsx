const StarIcon = ({ className, percentage, color }: { percentage?: number; color?: string; className?: string }) => {
    return (
        <svg xmlns="http://www.w3.org/2000/svg" className={className || "h-5 w-5"} viewBox="0 0 20 20" fill="currentColor">
            <defs>
                <linearGradient id="grad">
                    <stop offset={`${(percentage || .5) * 100}%`} stop-color={color ? color : "#4b5563"} />
                    <stop offset={`${100 - ((percentage || .5) * 100)}%`} stop-color="white" />
                </linearGradient>
            </defs>
